...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ( date_reservation_exists($date_reservation, $id_machine, $id_utilisateur)) {
        header("Location: ../views/cadet_view.php?id=$id_machine&error=exists");
        exit();
    }


    // Ajout de la reservation
    add_reservation($date_reservation, $id_machine, $id_utilisateur);
    $nom_machine = machine($id_machine);
...
try{
    ...
    $mail->Body = "RESERVATION TRANSMISE <b>machine : $nom_machine  ;Date de reservation : $date_reservation</b>";
    $mail->AltBody = "machine : $nom_machine  ;Date de reservation : $date_reservation";
    $mail->send();
    echo 'Message has been sent';
} catch (Exception $e) {
    echo "Message could not be sent. Mailer Error: {$mail->ErrorInfo}";
}
     header("Location: ../views/cadet_view.php?id=$id_machine&success=added");
    exit();
